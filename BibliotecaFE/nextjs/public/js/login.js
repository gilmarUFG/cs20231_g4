...
let direita = true;
if (!btnMovel) {
  console.log("Não foi possível encontrar o botão de mudar tela");
} else {
  ...
  function moverTelaCadastro() {
    if (direita) {
      ...
      telaMovel.style.animation =
        "moverparaLadoEsquerdo 1s cubic-bezier(.8,-0.40,.32,1.40)";
      ...
      telaMovel.style.animation =
        "moverparaLadoDireito 1s cubic-bezier(.8,-0.40,.32,1.40)";
  ...
  const btnMostrarcadastroSenha = document.getElementById(
    "id_btnMostrarSenhaCadastro"
  );
  const inputConfirmaCadastroSenha = document.getElementById(
    "id_confirmaCadastroSenha"
  );
  const btnMostrarconfirmaCadastroSenha = document.getElementById(
    "id_btnMostrarSenhaConfirmaCadastro"
  );
